...
const CategoryDistributionChart: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get<{ value: ProductData[] }>(`${baseUrl}/api/OrderControllers/GetTopPopularProducts`);
  ...
  // Map product names based on the current language
  const productNameField = `productName_${currentLanguage}` as keyof ProductData;

  // Ensure that data is an array before mapping
  const categories = Array.isArray(data) ? data.map((item) => item[productNameField] || 'Unknown') : [];
          ...
          {data.map((product, index) => (
              ...
              ></span>
                  <p className="flex w-full justify-between text-sm font-medium text-black dark:text-white">
                    <span>{product[productNameField] || 'Unknown'}</span>
