import 'tailwindcss/tailwind.css';
import { Link } from 'react-router-dom';
import {t} from "i18next";

export default function Page404() {
  return (
    <>
      <main className="min-h-[1200px] relative isolate min-h-full">
      <div className="bg-gray-100">
      <div className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        {/* <img
          src="https://images.unsplash.com/photo-1545972154-9bb223aac798?ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=3050&q=80&exp=8&con=-15&sat=-75"
          alt=""
          className="absolute inset-0 -z-10 h-full w-full object-cover object-top"
        /> */}
        <div className="mx-auto max-w-7xl px-6 py-32 text-center sm:py-40 lg:px-8">
          <p className="text-base font-semibold leading-8 text-indigo-500 sm:text-9xl">404</p>
          <h1 className="mt-4 text-3xl font-bold tracking-tight sm:text-5xl">{t('404_PageNotFound')}</h1>
          <p className="mt-4 text-base  sm:mt-6">{t('404_Sorry')}</p>
          <div className="mt-10 flex justify-center">
            <Link to={"/"} className="text-sm font-semibold leading-7 hover:text-indigo-300">
                <span aria-hidden="true">&larr;</span> {t('404_Back')}
            </Link>
          </div>
        </div>
        </div>
        </div>
      </main>
    </>
  )
}
